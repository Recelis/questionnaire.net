...
export const apiGetQuestionnaire = async (
    ...
    try {
        ...
        if (!res.ok) {
            const message = await res.text();
            throw new Error(`Failed to get questionnaire. Status: ${res.status}: ${message}`);
        }
...
export const apiDeleteQuestionnaire = async (questionnaireId: number): Promise<boolean> => {
    try {
        const baseUrl = import.meta.env.VITE_API_URL ?? DEFAULT_BASE_URL;
        const res = await fetch(`${baseUrl}/Questionnaire/${questionnaireId}`, {
            method: 'DELETE',
